...
package AGIHandlers::VDAD_ALL_outbound;

#
# VDAD_ALL_outbound.pm version 2.14
#
# Formally agi-VDAD_ALL_outbound.agi
# 
...
sub run {
	...
	# check if the AGIARGS variable is set and if so use that
	my $agi_arg_str =  $AGI->get_variable('AGIARGS');

	my $call_handle_method 		= 'NORMAL';
	my $agent_search_method		= 'LB';
	my $connected_line_name		= $calleridname;
	my $survey_first_audio_file	= '';
	my $survey_dtmf_digits		= '';
	my $survey_ni_digit		= '';
	my $survey_opt_in_audio_file	= '';
	my $survey_ni_audio_file	= '';
	my $survey_no_response_action	= '';
	my $survey_ni_status		= '';

	if (!defined $agi_arg_str) 
		{
		$call_handle_method		= $env->{'arg_1'} // 'NORMAL';
		$agent_search_method		= $env->{'arg_2'} // 'LB';
		$connected_line_name		= $env->{'arg_3'} // $calleridname;
		$survey_first_audio_file	= $env->{'arg_4'} // '';
		$survey_dtmf_digits		= $env->{'arg_5'} // '';
		$survey_ni_digit		= $env->{'arg_6'} // '';
		$survey_opt_in_audio_file	= $env->{'arg_7'} // '';
		$survey_ni_audio_file		= $env->{'arg_8'} // '';
		$survey_no_response_action	= $env->{'arg_9'} // '';
		$survey_ni_status		= $env->{'arg_10'} // '';
		}
	else 
		{
		my @agi_args = split(/,/, $agi_arg_str);
		$call_handle_method 		= $agi_args[0] // 'NORMAL';
		$agent_search_method		= $agi_args[1] // 'LB';
		$connected_line_name		= $agi_args[2] // $calleridname;
		$survey_first_audio_file	= $agi_args[3] // '';
		$survey_dtmf_digits		= $agi_args[4] // '';
		$survey_ni_digit		= $agi_args[5] // '';
		$survey_opt_in_audio_file	= $agi_args[6] // '';
		$survey_ni_audio_file		= $agi_args[7] // '';
		$survey_no_response_action	= $agi_args[8] // '';
		$survey_ni_status		= $agi_args[9] // '';
		}

	$fagi_channel = $channel;

	$script = 'VDAD_ALL_outbound';
	$AGILOG = '0';
